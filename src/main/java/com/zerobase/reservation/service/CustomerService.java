...
public class CustomerService {
  ...
  public String signIn(SignInForm form) { //로그인
    //return token
    Customer customer = validateSignIn(form).orElseThrow(
        () -> new CustomException(ErrorCode.NOT_FOUND_USER));//로그인정보없음
    return provider.createToken(customer.getEmail(), customer.getId(), TYPE);
  }

  public Reservation makeReservation(String token, Long shop_id,
      ReservationInputForm dto) { //shopId 를 통해 reservation 생성
    UserVo user = provider.getUserVo(token);
    Customer c = customerRepository.findByIdAndEmail(user.getId(), user.getEmail())
        .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_USER));
    Shop s = shopRepository.findById(shop_id).stream().findFirst()
    ...
    reservationRepository.findByCustomer(c).stream()
        ...
        .filter(reservation -> reservation.getReservationAt().truncatedTo(ChronoUnit.DAYS)
            .equals(LocalDateTime.parse(dto.getReservateAt(),
                DateTimeFormatter.ofPattern("yy.MM.dd HH:mm")).truncatedTo(ChronoUnit.DAYS)))
        .findAny()
  ...
  public void cancelReservation(String token, Long reservation_id) {
    ...
    Customer c = customerRepository.findByIdAndEmail(user.getId(), user.getEmail())
        .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_USER));
    Reservation r = reservationRepository.findById(reservation_id)
        .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_RESERVATION));
    if (r.getReservationAt().truncatedTo(ChronoUnit.DAYS)
