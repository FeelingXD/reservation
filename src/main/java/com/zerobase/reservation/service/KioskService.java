...
public class KioskService {
  ...
  public Kiosk addKiosk(String token, Long shopId) { //매니저토큰 필요! 상점 키오스크 추가하기 .
    ...
    if (!(Objects.equals(s.getManager().getId(), user.getId())) || !(Objects.equals(
        s.getManager().getEmail(), user.getEmail()))) {
  ...
  public Reservation checkReservation(Long kiosk_id, KioskInputForm form) { // 예약자 전화번호로 예약 승인하기
    var k = kioskRepository.findById(kiosk_id)
    ...
    var r = reservationRepository.findByCustomer_Phone(form.getCustomer_phone())
        ...
        .filter(reservation -> reservation.getReservationStatus()
            .equals(ReservationStatus.RESERVATION_COMPLETE))
        .filter(reservation -> (reservation.getReservationAt().truncatedTo(ChronoUnit.DAYS)).equals(
    ...
    if (!r.getReservationAt().truncatedTo(ChronoUnit.DAYS)
        .equals(LocalDateTime.now().truncatedTo(ChronoUnit.DAYS))) {// 예약일자가 다른경우
